...
$app->get('/api/tweets/', function() use($app) {
	$req = $app->request();
	$geocode = $req->get('geocode');
	$data = searchTweets($geocode);
	foreach($data->statuses as $status){
		$status->created_at_str = (new DateTime($status->created_at))->format('M jS g.i A');
		$status->twitter_name = $status->user->screen_name;
...
function searchTweets($geocode){
	...
	$encodedData = file_get_contents('https://api.twitter.com/1.1/search/tweets.json?q=&geocode='."$geocode", false, $context);
	return json_decode($encodedData);
};
